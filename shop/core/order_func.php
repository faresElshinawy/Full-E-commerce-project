...
function get_order_items($conn,$id){
    $sql = "SELECT products.pro_name ,products.price FROM pix.orders , pix.order_items , pix.users , pix.products WHERE  orders.user_id =  '$id' AND orders.order_id = order_items.order_id";
    $result = mysqli_query($conn,$sql);
    return $result ;
}
