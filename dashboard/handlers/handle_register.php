...
if(check_request_method('POST')){
    ...
    }elseif(check_unique_name($conn,$firstname,$lastname)){
    ...
    }elseif(check_unique_email($conn,$email)){
    ...
    if(require_input($gender)){
        ...
        if(require_input($permission)){
            $errors['permission'] = 'permission is require';
        }elseif(min_len($gender,1)){
            $errors['permission'] = 'not a valid permission info';
        }elseif(max_len($gender,1)){
            $errors['permission'] = 'not a valid permission info';
        }elseif(numeric($gender)){
            $errors['permission'] = 'not a valid permission info';
    ...
    if(empty($errors)){
        new_user_dash($conn,$firstname,$lastname,$email,$password,$phone,$address,$new_image_name,$city,$gender,$permission);
        $_SESSION['success'] = 'your account is done successfully';
        redirect('../users.php');
    }else{
