<?php 
    ...
    function users_data($conn){
        $sql = "SELECT user_id , first_name , last_name , email , password , phone , address , image , gender_name , city_name , permissions.per_name FROM pix.users , pix.genders , pix.cities , pix.permissions WHERE
    ...
    function new_user($conn , $firstname , $lastname , $email , $password , $phone , $address , $image , $city , $gender){
        ...
        // insert new user from dashboard function

        function new_user_dash($conn , $firstname , $lastname , $email , $password , $phone , $address , $image , $city , $gender , $per_id){
            $sql = "INSERT INTO `users`(`first_name`, `last_name`, `email`, `password`, `phone`, `address`, `image`, `city_id`, `gender_id`,`permission_id`) VALUES ('$firstname','$lastname','$email','$password','$phone','$address','$image','$city','$gender','$per_id')";
            mysqli_query($conn,$sql);
        }
